...
[reg_filter,binary_mask] = cellfun(@(reg_window_edge) get_reg_filter(img_support_sz, base_target_sz, params, reg_window_edge), reg_window_edge, 'uniformoutput', false);
...
%     xl{3}=xl{3}*0;
    ...
    for i=1:5
        binary_mask{i}=imresize(binary_mask{i},[filter_sz(i,1) filter_sz(i,2)],'nearest');
    end
    
    % do conjugate gradient
    [hf, flag, relres, iter, res_norms, p, rho] = pcg_ccot(...
        @(x) lhs_operation(x, samplesf, reg_filter, sample_weights, feature_reg,binary_mask),...
