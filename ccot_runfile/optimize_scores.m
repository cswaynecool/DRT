function [translation_vec, scale_ind] = optimize_scores(scores_fs, iterations, ky, kx, currentScaleFactor, opts, filter_sz)

scaleFactors=opts.scaleFactors;
target_sz=opts.target_sz;
img_support_sz=opts.img_support_sz;
...
% for each scale.
sampled_scores = prod(output_sz) * ifft2(scores_fs, 'symmetric');
...
trans_row = (mod(max_pos_y(1,1,scale_ind) + pi, 2*pi) - pi) / (2*pi) * output_sz(1);
trans_col = (mod(max_pos_x(1,1,scale_ind) + pi, 2*pi) - pi) / (2*pi) * output_sz(2);

translation_vec = round([trans_row, trans_col] .* (img_support_sz./output_sz) * currentScaleFactor * scaleFactors(scale_ind));
max_motion1=max(abs(trans_row),abs(trans_col)); max_motion2=max(abs(translation_vec));

 if max_motion1>filter_sz(1,1)/2*0.45&&max_motion2>60
