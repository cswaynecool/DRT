function [x,flag,relres,iter,resvec,p,rho] = pcg_ccot(A,b,tol,maxit,M1,M2,x0,p,rho,cnn_third,feature_input,varargin)
...
%         end
    ...
    rho1 = rho;
    rho = inner_product_cdcf1(r, z,cnn_third,feature_input);
    if ((rho == 0) || isinf(rho))
        flag = 4;
        break
    end
    ...
    pq = inner_product_cdcf1(p, q,cnn_third,feature_input);
%     if ((pq <= 0) || isinf(pq))
    if ((pq <= 0) || isinf(pq))
        flag = 4;
        break
