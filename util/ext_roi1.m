function [roi, roi_pos, preim, pad,roi1,roi2,pad1,roi_pos_large] = ext_roi1(im, GT, l_off, roi_size, r_w_scale)
[h, w, ~] = size(im);
win_w = GT(3);
win_h = GT(4);
win_lt_x = GT(1);
...
if clip<=0
    ...
    im(GT(2):GT(2)+GT(4)-1,GT(1):GT(1)+GT(3)-1,:)=target;
    im1(GT(2):GT(2)+GT(4)-1,GT(1):GT(1)+GT(3)-1,:)=imrotate(target,180);
    
else
    GT(1)=GT(1)+pad; GT(2)=GT(2)+pad;
    target=im1(GT(2):GT(2)+GT(4)-1,GT(1):GT(1)+GT(3)-1,:);
    ff=rand(size(target(:,:,1)))>0.8;
    ff=repmat(ff,[1 1 3]);
    im(GT(2):GT(2)+GT(4)-1,GT(1):GT(1)+GT(3)-1,:)=target.*ff;
    im1(GT(2):GT(2)+GT(4)-1,GT(1):GT(1)+GT(3)-1,:)=imrotate(target,180);
end
